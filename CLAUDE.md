...
./scripts/bump-version.sh patch   # 1.1.0 → 1.2.0 (修订版)
./scripts/bump-version.sh minor   # 1.1.0 → 2.0.0 (次版本)
...
GitHub Actions workflow (`.github/workflows/release.yml`) 会在 `v*` tag 推送时自动：
- 在 macOS-14 runner 上构建 Release 版本
- 打包为 DMG 文件
- 创建 GitHub Release 并上传 DMG

用户在设置页的"检查更新"按钮会查询 GitHub Releases API，发现新版本后可一键下载 DMG 安装。
